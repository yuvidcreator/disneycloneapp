        ...
        <div className="relative">
            ...
            {!session ? ( <Hero /> ) :
                ...
                <section className="relative z-50">
                    ...
                    <div className="absolute inset-y-48 md:inset-y-auto md:bottom-20 inset-x-4 md:inset-x-12 space-y-6 z-50">
                        ...
                        <div className="flex items-center space-x-3 md:space-x-5">
                            <button className="text-xs md:text-base bg-[#f9f9f9] text-black flex items-center justify-center py-2.5 px-6 rounded hover:bg-[#c6c6c6]">
                                <Image src="/images/play-icon-black.svg" alt="play-icon" width={30} height={30} 
                                className="h-6 md:h-8"
                                />
                                <span className="uppercase font-medium tracking-wide">Play</span>
                            </button>

                            <button className="text-xs md:text-base bg-black/30 text-[#f9f9f9] border border-white flex items-center justify-center py-2.5 px-6 rounded hover:bg-[#c6c6c6]"
                            onClick={() => setShowPlayer(true)}
                            >
                                <Image src="/images/play-icon-white.svg" alt="play-icon" width={30} height={30} 
                                className="h-6 md:h-8"
                                />
                            ...
                            <div className="rounded-full border-2 border-white flex items-center justify-center w-11 h-11 cursor-pointer bg-black/60">
                                <Image src="/images/group-icon.svg" alt="group-icon" width={44} height={44} />
                            </div>
                        </div>

                        <p className='text-sm md:text-sm'>
                            {result.release_date || result.first_air_date } &bull;{" "}
                            {Math.floor(result.runtime / 60)}h {result.runtime % 60}m &bull;{" "} 
...
export async function getServerSideProps(context) {
    ...
    const request = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.MV_API_KEY}&language=en-US&append_to_response=videos`).then((response) => response.json());
